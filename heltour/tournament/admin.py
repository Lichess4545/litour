...
class SeasonAdmin(_BaseAdmin):
    ...
    def generate_invite_codes(self, request, queryset):
        ...
        if season.league.registration_mode != RegistrationMode.INVITE_ONLY:
            self.message_user(request, f'{season.league.name} is not configured for invite-only registration.',
                              messages.ERROR)
            return
    ...
    def generate_invite_codes_view(self, request, object_id):
        ...
        if season.league.registration_mode != RegistrationMode.INVITE_ONLY:
            messages.error(request, f'{season.league.name} is not configured for invite-only registration.')
            return redirect('admin:tournament_season_changelist')
        ...
        if request.method == 'POST':
            try:
                ...
                if count < 1 or count > 10000:
                    ...
                    messages.success(
                        request, f'Successfully generated {len(codes)} {code_type} invite codes.'
                    )
...
class TeamAdmin(_BaseAdmin):
    ...
    actions = ['update_board_order_by_rating', 'create_slack_channels', 'generate_team_invite_codes']
    league_id_field = 'season__league_id'
    ...
    def generate_team_invite_codes(self, request, queryset):
        if queryset.count() > 1:
            self.message_user(request, 'Invite codes can only be generated for one team at a time.',
                              messages.ERROR)
            return
        team = queryset[0]
        if team.season.league.registration_mode != RegistrationMode.INVITE_ONLY:
            self.message_user(request, f'{team.season.league.name} is not configured for invite-only registration.',
                              messages.ERROR)
            return
    ...
    def get_urls(self):
        ...
        my_urls = [
            path('<int:object_id>/generate_team_invite_codes/',
    ...
    def generate_team_invite_codes_view(self, request, object_id):
        ...
        if not is_captain and not request.user.has_perm('tournament.change_team', team.season.league):
            raise PermissionDenied
        ...
        if request.method == 'POST':
            try:
                ...
                if count < 1:
                    ...
                    messages.error(request, f'You can only generate {remaining_allowed} more codes (maximum {max_codes} total)')
                else:
                    codes = InviteCode.create_batch(
                        league=team.season.league,
                        season=team.season,
                    ...
                    messages.success(
                        request, f'Successfully generated {len(codes)} team member invite codes.'
                    )
                    return redirect('admin:tournament_invitecode_changelist')
        ...
        return render(request, 'tournament/admin/generate_team_invite_codes.html', context)
...
class InviteCodeAdmin(_BaseAdmin):
    list_display = ('code', 'code_type', 'league', 'season', 'team', 'used_by', 'used_at', 'created_by')
    ...
    def export_codes(self, request, queryset):
        ...
        writer.writerow(['Code', 'Type', 'League', 'Season', 'Team', 'Status', 'Used By', 'Used At', 'Created By', 'Notes'])

...
class SeasonAdmin(_BaseAdmin):
    ...
    def generate_invite_codes(self, request, queryset):
        ...
        if season.league.registration_mode != RegistrationMode.INVITE_ONLY:
            self.message_user(
                request, "This league is not configured as invite-only.", messages.ERROR
            )
            return
    ...
    def generate_invite_codes_view(self, request, object_id):
        ...
        if request.method == "POST":
            try:
                ...
                if count <= 0:
                    messages.error(
                        request, "Please enter a positive number of codes to generate."
                    )
                elif count > 10000:
                    messages.error(
                        request, "Cannot generate more than 10,000 codes at once."
                    )
                else:
                    # Generate the codes
                    ...
                    messages.success(
                        request, f"Successfully generated {len(codes)} invite codes."
                    )

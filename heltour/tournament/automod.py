...
def automod_noshow(pairing, **kwargs):
    if (
        (
            pairing.game_link
            and (not pairing.white_confirmed or not pairing.black_confirmed)
        )
        or pairing.tv_state == "has_moves"
        or pairing.result
    ):
        ...
        return
    if (
        pairing.white_confirmed
        and pairing.black_confirmed
        and pairing.game_id() is not None
    ):

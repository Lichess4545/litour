...
def team_tournament_to_structure(season) -> Tournament:
    ...
    format_type = (
        ...
        if season.league.pairing_type in ["knockout-single", "knockout-multi"]
        else TournamentFormat.SWISS
    )
...
def lone_tournament_to_structure(season) -> Tournament:
    ...
    for round_obj in season.round_set.filter(is_completed=True).order_by("number"):
        ...
        if format_type == TournamentFormat.SWISS:
            ...
            byes = {b.player_id: b for b in PlayerBye.objects.filter(round=round_obj)}
...
def knockout_bracket_to_structure(knockout_bracket) -> Tournament:
    ...
    seedings = (
        KnockoutSeeding.objects.filter(bracket=knockout_bracket)
        .order_by("seed_number")
        .select_related("team" if season.league.competitor_type == "team" else None)
    )
    ...
    updated_tournament = Tournament(
        ...
        current_match_number=_calculate_current_match_number(
            tournament.rounds, knockout_bracket.matches_per_stage
        ),
    )
...
def multi_match_knockout_to_structure(season) -> Tournament:
    ...
    for group_id, group_rounds in round_groups.items():
        if len(group_rounds) == 1:
            ...
            existing_round = next(
                (r for r in tournament.rounds if r.number == round_obj.number), None
            )
            if existing_round:
                aggregated_rounds.append(existing_round)
        else:
            # Multiple rounds - aggregate them
            aggregated_round = _aggregate_multi_match_rounds(
                group_rounds, tournament, season
            )
...
def _aggregate_multi_match_rounds(group_rounds, base_tournament, season):
    ...
    for round_obj in group_rounds:
        round_structure = next(
            (r for r in base_tournament.rounds if r.number == round_obj.number), None
        )
        if round_structure:
...
def _calculate_current_match_number(rounds, matches_per_stage):
    ...
    estimated_total_pairs = (
        total_matches // matches_per_stage if total_matches > 0 else 1
    )

    if estimated_total_pairs == 0:
        return 1

    return _get_current_match_number_from_matches(
        latest_round.matches, estimated_total_pairs
    )
        ...
        for original_pairing_order in range(1, total_pairs + 1):
            ...
            if original_match:
                try:
                    ...
                    progress_a, created = (
                        TeamMultiMatchProgress.objects.update_or_create(
                            bracket=bracket,
                            team=team_a,
                            round_number=round_obj.number,
                            defaults={
                                "stage_name": round_obj.knockout_stage
                                or f"round_{round_obj.number}",
                                "opponent_team": team_b,
                                "original_pairing_order": original_pairing_order,
                                "matches_completed": stage_status[
                                    "completed_current_match"
                                ],

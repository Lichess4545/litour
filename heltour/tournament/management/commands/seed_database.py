...
class Command(BaseCommand):
                ...
                for league in leagues:
                    for i, season_config in enumerate(season_configs):
                        ...
                        builder.season(
                            ...
                            registration_open=season_config.get(
                                "registration_open", False
                            ),
                            nominations_open=season_config.get(
                                "nominations_open", False
                            ),
                        )
                        ...
                        if league.is_team_league():
                            ...
                            num_players_for_season = min(
                                len(player_data), max(12, len(player_data) // 3)
                            )
                        ...
                        if season_config.get("is_active") or season_config.get(
                            "is_completed"
                        ):
                            rounds_to_complete = (
                                season_config["rounds"]
                                if season_config.get("is_completed")
                            ...
                            for round_num in range(1, rounds_to_complete + 1):
                                ...
                                round_obj = builder.start_round(
                                    round_num, generate_pairings_auto=False
                                )
                                ...
                                if (
                                    season_config.get("is_completed")
                                    or round_num < rounds_to_complete
                                ):
                        ...
                        league_type = (
                            "team" if league.is_team_league() else "individual"
                        )
                        self.stdout.write(
                            self.style.SUCCESS(
        ...
        for i in range(0, len(teams) - 1, 2):
            if i + 1 < len(teams):
                ...
                for board_num in range(1, round_obj.season.boards + 1):
                    if board_num <= len(white_members) and board_num <= len(
                        black_members
                    ):
        ...
        for team in teams:
            if team.id not in paired:
                TeamBye.objects.create(
                    round=round_obj, team=team, type="full-point-pairing-bye"
                )
    ...
    def _create_manual_lone_pairings(self, round_obj):
        ...
        season_players = list(
            SeasonPlayer.objects.filter(season=round_obj.season, is_active=True)
        ...
        for sp in season_players:
            if sp.player_id not in paired:
                PlayerBye.objects.create(
                    round=round_obj, player=sp.player, type="full-point"
                )
...
class PlayerNameGenerator:
    ...
    def generate_players(
        self, count: int, rating_type: str = "standard", season_suffix: str = ""
    ) -> list:
        ...
        for i in range(count):
            ...
            players.append({"name": username, "rating": rating})

...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--list", action="store_true", help="List available predefined tournaments"
        )
    ...
    def handle(self, *args, **options):
        ...
        if tournament_arg in predefined:
            ...
            if not league_tag:
                ...
                if basename.endswith(".trf"):
                    ...
                    raise CommandError(
                        "--league-tag is required when using a custom TRF16 file"
                    )
        else:
        ...
        try:
            if mode == "teams":
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully seeded teams and round 1 pairings for {season.name}"
                    )
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully seeded teams and round 1 with results for {season.name}"
                    )
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Successfully seeded complete tournament: {season.name}"
                    )
                )

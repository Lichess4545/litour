...
def _lichess_message(league, username, subject, text):
    if league.enable_notifications:
        try:
            ...
            logger.error(
                f"[Error] Sending lichess messsage failed with reason: {str(e)}"
            )
...
def player_account_status_changed(instance, old_value, new_value, **kwargs):
    ...
    pending_regs = (
        Registration.objects.filter(
            player__lichess_username__iexact=instance.lichess_username, status="pending"
        )
...
def alternate_assigned(season, alt_assignment, **kwargs):
    ...
    if aa.player == aa.replaced_player:
        message = (
            '%sI have reassigned <@%s> to play on board %d of "%s" for round %d.%s'
            % (
...
def notify_players_game_scheduled(round_, pairing, **kwargs):
    ...
    confirm_url = abs_url(
        reverse("by_league:confirm_scheduled_time", args=[league.tag])
    )
...
def notify_players_game_started(
    *, pairing, do_clockstart, clockstart_in, gameid, **kwargs
):
    im_msg = (
        "We created your game.\n"
    ...
    li_msg = ""
    send_pairing_notification(
        "game_started", pairing, im_msg, mp_msg, li_subject, li_msg
    )

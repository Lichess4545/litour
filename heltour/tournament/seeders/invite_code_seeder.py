...
class InviteCodeSeeder(BaseSeeder):
    ...
    def seed(
        self, captain_codes: int = 10, seasons: Optional[List[Season]] = None, **kwargs
    ) -> List[InviteCode]:
    ...
    def seed_team_member_codes(
        self, teams: Optional[List[Team]] = None, codes_per_team: int = 3, **kwargs
    ) -> List[InviteCode]:
        ...
        if teams is None:
            ...
            team_ids = (
                TeamMember.objects.filter(is_captain=True)
        ...
        for team in teams:
            ...
            existing_codes = InviteCode.objects.filter(
                season=season, created_by_captain=captain
            ).count()
            ...
            for i in range(codes_to_create):
                ...
                if used:
                    ...
                    available_players = Player.objects.filter(is_active=True).exclude(

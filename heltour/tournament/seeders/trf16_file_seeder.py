...
def seed_complete_tournament(trf16_path, league_tag, existing_league=None):
    ...
    """
    print(
        f"=== Seeding complete tournament from {trf16_path} (league: {league_tag}) ==="
    )
    ...
    for i, (team_id, score) in enumerate(sorted_teams, 1):
        ...
        if team_name:
            print(
                f"{i:2d}. {team_name:30s} - MP: {score.match_points:.1f}, GP: {score.game_points:.1f}"
            )
...
@transaction.atomic
def seed_partial_tournament(
    trf16_path, league_tag, num_rounds=1, include_results=True, existing_league=None
):
    """
    ...
    """
    print(
        f"=== Seeding {num_rounds} round(s) {'with results' if include_results else 'without results'} ==="
    )
    ...
    converter.add_rounds_to_builder(
        builder, max_rounds=num_rounds, include_results=include_results
    )

...
class DbToStructureIntegrationTests(TestCase):
    ...
    def test_simple_tournament_scores(self):
        ...
        TeamBye.objects.create(
            round=round1, team=self.teams[2], type="full-point-pairing-bye"
        )

        # Round 2: Team 1 draws Team 3 (1-1), Team 2 gets bye
        round2 = Round.objects.create(season=self.season, number=2, is_completed=False)
        self.create_pairing_with_results(round2, 0, 2, ["1-0", "1-0"])
        TeamBye.objects.create(
            round=round2, team=self.teams[1], type="full-point-pairing-bye"
        )

        # Round 3: Team 2 draws Team 3 (1-1), Team 1 gets bye
        ...
        TeamBye.objects.create(
            round=round3, team=self.teams[0], type="full-point-pairing-bye"
        )

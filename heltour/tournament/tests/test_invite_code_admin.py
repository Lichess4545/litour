...
class InviteCodeAdminTestCase(TestCase):
    ...
    def test_season_admin_generate_invite_codes_action(self):
        ...
        result = admin.generate_invite_codes(request, Season.objects.filter(pk=open_season.pk))
        self.assertIsNone(result)  # Action returns None when showing error
        
        # Test with invite-only league
        result = admin.generate_invite_codes(request, Season.objects.filter(pk=self.season.pk))
    ...
    def test_team_admin_generate_invite_codes_action(self):
        ...
        # Test the action
        result = admin.generate_team_invite_codes(request, Team.objects.filter(pk=self.team.pk))
        self.assertEqual(result.status_code, 302)  # Should redirect
    ...
    def test_invite_code_admin_export_codes(self):
        ...
        self.assertIn('attachment; filename="invite_codes.csv"', response['Content-Disposition'])

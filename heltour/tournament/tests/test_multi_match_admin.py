...
class MultiMatchKnockoutBracketAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        TeamMember.objects.create(
            team=self.team_a, player=self.player_a1, board_number=1
        )
        TeamMember.objects.create(
            team=self.team_a, player=self.player_a2, board_number=2
        )
        TeamMember.objects.create(
            team=self.team_b, player=self.player_b1, board_number=1
        )
        TeamMember.objects.create(
            team=self.team_b, player=self.player_b2, board_number=2
        )
    ...
    def test_tournament_type_display(self):
        ...
        bracket_single = KnockoutBracket.objects.create(
            season=self.season, bracket_size=4, matches_per_stage=1
        )

        # Return matches
        bracket_return = KnockoutBracket.objects.create(
            season=season2, bracket_size=4, matches_per_stage=2
        )

        # Triple matches
        bracket_triple = KnockoutBracket.objects.create(
            season=season3, bracket_size=4, matches_per_stage=3
        )

        self.assertEqual(
            self.admin.tournament_type(bracket_single), "Single Elimination"
        )
    ...
    def test_generate_next_match_set_action_multiple_selection(self):
        ...
        bracket1 = KnockoutBracket.objects.create(
            season=self.season, bracket_size=4, matches_per_stage=2
        )
...
class MultiMatchProgressAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        self.bracket = KnockoutBracket.objects.create(
            season=self.season, bracket_size=2, matches_per_stage=3
        )
...
class MultiMatchAdminIntegrationTest(TestCase):
    ...
    def setUp(self):
        ...
        KnockoutSeeding.objects.create(
            bracket=self.bracket, team=self.team_a, seed_number=1
        )
        KnockoutSeeding.objects.create(
            bracket=self.bracket, team=self.team_b, seed_number=2
        )

        # Create round
        self.round = Round.objects.create(
            season=self.season, number=1, is_completed=False, knockout_stage="finals"
        )
    ...
    def test_create_next_match_pairings_helper(self):
        ...
        return_pairing = TeamPairing.objects.filter(
            round=self.round, pairing_order=2
        ).first()

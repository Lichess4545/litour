...
class RegistrationViewIntegrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="testviewplayer", password="testpass123"
        )
        self.player = Player.objects.create(
            lichess_username="testviewplayer", rating=1700, email="viewtest@example.com"
        )
    ...
    def test_registration_with_captain_code_full_flow(self):
        ...
        success_url = leagueurl(
            "registration_success", self.league.tag, self.season.tag
        )
    ...
    def test_registration_with_team_member_code_full_flow(self):
        ...
        existing_team = Team.objects.create(
            season=self.season, number=1, name="Existing View Team", is_active=True
        )

        # Add captain
        captain = Player.objects.create(lichess_username="viewcaptain", rating=2000)
    ...
    def test_registration_without_invite_code_shows_pending(self):
        ...
        # Should redirect to success page
        success_url = leagueurl(
            "registration_success", regular_league.tag, regular_season.tag
        )
        self.assertRedirects(response, success_url)
        ...
        registration = Registration.objects.get(
            player=self.player, season=regular_season
        )
    ...
    def test_team_details_link_works(self):
        ...
        member = Player.objects.create(lichess_username="testmember", rating=1600)

        # Add member to team
        TeamMember.objects.create(
            team=team, player=member, board_number=2, is_captain=False
        )
        ...
        team_url = leagueurl(
            "team_profile", self.league.tag, self.season.tag, team.number
        )
...
class RegistrationErrorHandlingTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(username="errortest", password="testpass")
        self.player = Player.objects.create(lichess_username="errortest", rating=1600)
    ...
    def test_registration_with_used_code_shows_error(self):
        ...
        used_player = Player.objects.create(lichess_username="usedplayer", rating=1700)
        ...
        self.assertFormError(
            response, "form", "invite_code", "This invite code has already been used"
        )

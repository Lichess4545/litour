...
from heltour.tournament.models import (
    League, Season, Round, Team, TeamScore, TeamPairing, TeamMember,
    TeamPlayerPairing, Player, SeasonPlayer, PlayerPairing, TEAM_TIEBREAK_OPTIONS
)
from heltour.tournament.tests.testutils import (
    createCommonLeagueData, create_reg, get_league, get_player,
    get_round, get_season, set_rating
)
...
class TeamTiebreakTestCase(TestCase):
    ...
    def test_match_points_calculation(self):
        ...
        self.create_pairing_with_results(
            self.rounds[0], self.teams[0], self.teams[1],
            ['1-0', '1/2-1/2']
        )
        ...
        scores = {ts.team.number: ts for ts in TeamScore.objects.filter(team__season=self.season)}
    ...
    def test_sonneborn_berger_calculation(self):
        ...
        self.create_pairing_with_results(
            self.rounds[1], self.teams[0], self.teams[2],
            ['1-0', '0-1']  # Draw 1-1
        )
    ...
    def test_configurable_tiebreak_order(self):
        ...
        self.assertEqual(sort_key[4], team_score.game_points)  # Third configured tiebreak
        self.assertEqual(sort_key[5], team_score.head_to_head)  # Fourth configured tiebreak
    ...
    def test_standings_sort_order(self):
        ...
        sorted_scores = sorted(team_scores, key=lambda ts: ts.pairing_sort_key(), reverse=True)
        winner_index = None
        loser_index = None
    ...
    def test_tiebreak_sorting_when_tied(self):
        ...
        self.assertEqual(scores_dict[self.teams[2]].game_points, 1.0)  # 2 boards / 2 = 1
        ...
        sorted_scores = sorted(scores, key=lambda ts: ts.pairing_sort_key(), reverse=True)

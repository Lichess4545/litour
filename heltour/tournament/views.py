...
class LeagueHomeView(LeagueView):
    ...
    def lone_view(self):
        ...
        first_tiebreak = (
            (lone_tiebreaks[0], tiebreak_names.get(lone_tiebreaks[0], "TB"))
            if lone_tiebreaks
            else None
        )
...
class TeamPairingBlockView(PairingsView):
    ...
    def view(self, pairing_id):
        ...
        matching = next(
            (
                pl
                for pl in context["pairing_lists"]
        ...
        if reg_id:
            try:
                ...
                if registration.invite_code_used:
                    if (
                        registration.invite_code_used.code_type == "captain"
                        and self.league.is_team_league()
                    ):
...
class RostersView(SeasonView):
    def view(self):
        ...
        if self.season.league.pairing_type.startswith("knockout"):
            return redirect(
                "by_league:by_season:knockout_bracket",
                league_tag=self.league.tag,
                season_tag=self.season.tag,
            )
        ...
        def _view(league_tag, season_tag, user_data, can_edit, show_gender):
            ...
            teams = sorted(
                teams, key=lambda team: team.average_rating() or 0, reverse=True
            )
...
class StandingsView(SeasonView):
    ...
    def team_view(self):
        ...
        def _view(league_tag, season_tag, user_data):
            ...
            team_scores = list(
                enumerate(sorted(raw_team_scores, key=sort_key, reverse=True), 1)
            )
...
class ActivePlayerTableView(LeagueView):
    ...
    def view(self, page: int = 1):
        ...
        seasondatafull = (
            SeasonPlayer.objects.filter(player__pk__in=pks, season__league=self.league)
            .values("player__pk")
            .annotate(
        ...
        seasondata = {
            pl["player__pk"]: [pl["player__lichess_username"], pl["season_count"]]
            for pl in seasondatafull
        }

        oneplayer = namedtuple(
            "oneplayer",
            ["game_count", "lichess_username", "season_count", "last_played"],
        )
        subtable = []
        ...
        if page == 1:
            ...
            total_games = int(
                total_games / 2
            )  # there's always 2 players that play the game.
...
class LeagueDashboardView(LeagueView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            "advance_tournament" in request.POST
        ...
        if (
            "create_missing_matches" in request.POST
            and self.season.league.pairing_type.startswith("knockout")
        ):
    ...
    def _handle_cache_clear(self):
        ...
        try:
            ...
            try:
                ...
                messages.success(
                    self.request,
                    "All caches cleared successfully (Django cache + cacheops)!",
                )
            except ImportError:
    ...
    def _common_context(self):
        ...
        recent_task_results = list(TaskResult.objects.order_by("-date_created")[:25])
    ...
    def _get_knockout_advancement_info(self):
        ...
        if is_multi_match:
            ...
            rounds_with_pairings = (
                Round.objects.filter(season=self.season, teampairing__isnull=False)
                .distinct()
                .order_by("-number")
            )
            ...
            if rounds_with_pairings.exists():
                ...
                current_round = (
                    Round.objects.filter(season=self.season).order_by("number").first()
                )
            ...
            if existing_pairings == 0:
                ...
                try:
                    ...
                    active_teams = Team.objects.filter(
                        season=current_round.season, is_active=True
                    ).count()
                    expected_team_pairs = active_teams // 2
                    total_matches_expected = (
                        expected_team_pairs * bracket.matches_per_stage
                    )
                except Exception:
        ...
        if is_multi_match:
            try:
                multi_match_info = self._get_multi_match_completion_info(
                    current_round, bracket
                )
            except Exception as e:
                # If we can't get multi-match info (e.g., no pairings exist), create a minimal info object
                logger.error(
                    f"ERROR: Could not get multi-match info for round {current_round.number}: {str(e)}"
                )
                import traceback
        ...
        if is_multi_match and multi_match_info:
            ...
            all_existing_complete = (
                multi_match_info["total_matches_actual"] > 0  # Have some matches
                and multi_match_info["total_matches_completed"]
                == multi_match_info["total_matches_actual"]  # All complete
            )
            has_missing_matches = (
                multi_match_info["total_matches_actual"]
                < multi_match_info["total_matches_expected"]
            )
        ...
        round_to_advance = (
            last_completed_round if last_completed_round else current_round
        )
        ...
        if self.league.competitor_type == "team":
            if is_multi_match:
                ...
                tied_matches = self._get_multi_match_tied_pairs(
                    round_to_advance, bracket
                )
            else:
        ...
        if not can_advance:
            reason = (
                f"{len(tied_matches)} tied match(es) require manual tiebreak resolution"
            )
    ...
    def _handle_knockout_advancement(self):
        ...
        try:
            ...
            if not advancement_info["can_advance"]:
                messages.error(
                    self.request,
                    f"Cannot advance tournament: {advancement_info['reason']}",
                )
                return redirect(self.request.path)
            ...
            if advancement_info["is_final_round"]:
                    ...
                    if self.league.competitor_type == "team":
                        ...
                        if bracket.matches_per_stage > 1:
                            ...
                            from heltour.tournament.pairinggen import (
                                _get_multi_match_winners,
                            )

                            winners = _get_multi_match_winners(
                                last_completed_round, bracket
                            )
                        else:
                            ...
                            for pairing in team_pairings:
                                if pairing.black_team_id is None:
                                    ...
                                    if pairing.manual_tiebreak_value is not None:
                                        ...
                                        pairing.white_points is not None
                                        and pairing.black_points is not None
                                    ):
                        ...
                        if not from_stage:
                            ...
                            from heltour.tournament_core.knockout import (
                                get_knockout_stage_name as calc_stage_name,
                            )

                            teams_remaining = bracket.bracket_size // (
                                2 ** (last_completed_round.number - 1)
                            )
                        ...
                        messages.success(
                            self.request, "Tournament standings finalized!"
                        )
                ...
                if next_round:
                    ...
                    messages.success(
                        self.request,
                        "Tournament has been completed - all rounds finished",
                    )
    ...
    def _get_multi_match_completion_info(self, round_obj, bracket):
        ...
        team_pairings = TeamPairing.objects.filter(round=round_obj).select_related(
        ...
        if expected_team_pairs == 0:
            try:
                ...
                expected_team_pairs = (
                    active_teams // 2
                )  # Each pair plays against each other
            except Exception:
                expected_team_pairs = 0
        ...
        for team_key, pairings in team_pair_groups.items():
            ...
            for pairing in pairings:
                ...
                completed_boards = (
                    board_pairings.filter(result__isnull=False)
                    .exclude(result="")
                    .count()
                )
                is_match_completed = (
                    completed_boards > 0 and completed_boards == board_count
                )
            ...
            if (
                completed_matches == len(pairings)
                and len(pairings) == matches_per_stage
            ):
        ...
        is_complete = (
            completed_team_pairs == expected_team_pairs
            and total_matches_actual == total_matches_expected
        )
        ...
        if total_matches_actual == 0:
            ...
            status_message = (
                f"All {matches_per_stage} matches complete - ready to advance"
            )
            ...
            if total_matches_actual % expected_team_pairs == 0:
                ...
                if completed_match_sets == matches_per_stage:
                    ...
                    status_message = (
                        f"Match {completed_match_sets} of {matches_per_stage} complete"
                    )
                else:
            ...
            status_message = "Ready to create initial matches"
    ...
    def _get_multi_match_tied_pairs(self, round_obj, bracket):
        ...
        for team_key, pairings in team_pair_groups.items():
            ...
            if (
                ...
                and not has_manual_tiebreak
                and len(pairings) == bracket.matches_per_stage
                and abs(total_team1_points - total_team2_points) < 0.001
            ):  # Float comparison
                tied_matches.append(
                    {
    ...
    def _handle_create_missing_matches(self):
        ...
        try:
            ...
            try:
                ...
                for pairing in problem_pairings:
                    if pairing.teamplayerpairing_set.count() == 0:
                        logger.warning(
                            f"Found pairing without board pairings: {pairing.white_team.name} vs {pairing.black_team.name}"
                        )
                        # Create board pairings for this pairing
                        self._create_board_pairings_for_knockout_pairing(
                            pairing, active_round.season.boards
                        )
                        fixed_count += 1

                if fixed_count > 0:
                    logger.info(
                        f"Fixed {fixed_count} pairings with missing board pairings"
                    )
                    # Try again
                    try:
                        multi_match_info = self._get_multi_match_completion_info(
                        ...
                        messages.error(
                            self.request,
                            "Unable to analyze round status. Please check for pairings without board results.",
                        )
                        return redirect(self.request.path)
                else:
            ...
            logger.info(
                f"Expected: {multi_match_info['total_matches_expected']}, Actual: {multi_match_info['total_matches_actual']}"
            )
            logger.info(
                f"Expected team pairs: {multi_match_info['expected_team_pairs']}"
            )
            ...
            if (
                ...
                logger.warning(
                    f"Condition failed: {multi_match_info['total_matches_actual']} >= {multi_match_info['total_matches_expected']}"
                )
                messages.info(
                    self.request,
                    f"All required matches already exist ({multi_match_info['total_matches_actual']}/{multi_match_info['total_matches_expected']})",
                )
                return redirect(self.request.path)

            # Create missing matches
            created_count = self._create_missing_knockout_matches(
                active_round, bracket, multi_match_info
            )
            ...
            logger.info(
                "Checking for recently created pairings without board pairings..."
            )
            ...
            for p in recent_pairings:
                ...
                if board_count == 0:
                    logger.error(
                        f"Found recent pairing without boards: ID {p.id}, {p.white_team.name} vs {p.black_team.name if p.black_team else 'BYE'}"
                    )
    ...
    def _create_missing_knockout_matches(self, round_obj, bracket, multi_match_info):
        ...
        team_pair_groups = defaultdict(list)
        existing_pairings = TeamPairing.objects.filter(round=round_obj).select_related(
            "white_team", "black_team"
        )

        logger.info(
            f"Found {existing_pairings.count()} existing pairings in round {round_obj.number}"
        )
        ...
        if original_is_completed:
            logger.info(
                "Temporarily marking round as incomplete to prevent calculate_scores during pairing creation"
            )
        ...
        try:
            for pairing in existing_pairings:
                if pairing.black_team:  # Skip byes
                    team_key = tuple(
                        sorted([pairing.white_team.id, pairing.black_team.id])
                    )
                    team_pair_groups[team_key].append(pairing)
                    logger.info(
                        f"Added pairing {pairing.white_team.name} vs {pairing.black_team.name} to group {team_key}"
                    )
            ...
            with transaction.atomic():
                ...
                if len(team_pair_groups) == 0:
                    logger.info(
                        "No existing pairings found - creating initial tournament pairings from seedings"
                    )
                    created_count = self._create_initial_knockout_pairings(
                        round_obj, bracket
                    )
                else:
                    ...
                    for team_key, existing_pairings_list in team_pair_groups.items():
                        ...
                        if needed_matches > 0:
                            ...
                            logger.info(
                                f"Creating {needed_matches} matches for {team1.name} vs {team2.name}"
                            )

                            # Determine next pairing order
                            max_pairing_order = (
                                TeamPairing.objects.filter(round=round_obj).aggregate(
                            ...
                            logger.info(
                                f"DEBUG: Current max_pairing_order: {max_pairing_order}, created_count: {created_count}"
                            )
                            ...
                            for i in range(needed_matches):
                                ...
                                match_number_in_series = (
                                    len(existing_pairings_list) + i + 1
                                )  # 1-based
                                ...
                                logger.info(
                                    f"Creating match {match_number_in_series}: {white_team.name} (white) vs {black_team.name} (black)"
                                )
                                    ...
                                    with db_transaction.atomic():
                                        with reversion.create_revision():
                                            reversion.set_comment(
                                                "Created missing multi-match pairing."
                                            )
                                            calculated_pairing_order = (
                                                max_pairing_order + created_count + 1
                                            )
                                            ...
                                            logger.info(
                                                f"DEBUG: Created pairing: {white_team.name} vs {black_team.name}, pairing_order={calculated_pairing_order} (max_pairing_order={max_pairing_order}, created_count={created_count})"
                                            )

                                            # Create board pairings immediately in the same transaction
                                            self._create_board_pairings_for_knockout_pairing(
                                                team_pairing, round_obj.season.boards
                                            )

                                            # Verify board pairings were created
                                            board_count = team_pairing.teamplayerpairing_set.count()
                                            logger.info(
                                                f"Created {board_count} board pairings for pairing {team_pairing.id}"
                                            )

                                            if board_count == 0:
                                                raise ValueError(
                                                    f"Failed to create board pairings for {white_team.name} vs {black_team.name}"
                                                )

                                            created_count += 1
                                except Exception as e:
                                    logger.error(
                                        f"Failed to create pairing for {white_team.name} vs {black_team.name}: {str(e)}"
                                    )
                                    raise
        finally:
            ...
            if original_is_completed:
                ...
                if created_count > 0:
                    logger.info(
                        "Recalculating scores after creating all pairings with board pairings"
                    )
    ...
    def _create_board_pairings_for_knockout_pairing(self, team_pairing, board_count):
        ...
        white_player_list = self._get_player_list_for_team(
            team_pairing.white_team, team_pairing.round, board_count
        )
        black_player_list = self._get_player_list_for_team(
            team_pairing.black_team, team_pairing.round, board_count
        )

        for board_number in range(1, board_count + 1):
            white_player = (
                white_player_list[board_number - 1]
                if board_number <= len(white_player_list)
                else None
            )
            black_player = (
                black_player_list[board_number - 1]
                if board_number <= len(black_player_list)
                else None
            )
    ...
    def _get_player_list_for_team(self, team, round_obj, board_count):
        ...
        for board_number in range(1, board_count + 1):
            member = TeamMember.objects.filter(
                team=team, board_number=board_number
            ).first()
            if member:
    ...
    def _create_initial_knockout_pairings(self, round_obj, bracket):
        ...
        if len(seeded_teams) % 2 != 0:
            raise ValueError(
                f"Cannot create pairings with odd number of teams: {len(seeded_teams)}"
            )
        ...
        for team1, team2 in team_pairs:
            with reversion.create_revision():
                ...
                if board_count == 0:
                    raise ValueError(
                        f"Failed to create board pairings for {team1.name} vs {team2.name}"
                    )
    ...
    def view(self):
        ...
        if self.league.is_team_league():
            for season, sp in active_seasons_with_sp:
                ...
                team_member = (
                    TeamMember.objects.filter(player=player, team__season=season)
                    .select_related("team")
                    .first()
                )
                ...
                if team_member:
                    team_membership_by_season[season] = {
                        ...
                        "can_manage": team_member.is_captain
                        or team_member.is_vice_captain,
                    }
...
class TeamCompositionView(LeagueView):
    def view(self):
        ...
        teams = (
            Team.objects.filter(season=self.season, is_active=True)
...
class GameIdsView(LeagueView):
    def view(self):
        ...
        for round_obj in rounds:
            round_data = {"round_number": round_obj.number, "matches": {}}
            ...
            if self.league.is_team_league():
                ...
                try:
                    ...
                    is_multi_match = (
                        "return" in self.season.name.lower()
                        or "multi" in self.season.name.lower()
                    )
                    max_matches = 2 if is_multi_match else 1

                # Team tournament - group by match number
                team_pairings = TeamPairing.objects.filter(round=round_obj).order_by(
                    "pairing_order"
                )
                ...
                if team_pairings.exists():
                    ...
                    for p in team_pairings:
                        unique_pairs.add(
                            (
                                min(p.white_team_id, p.black_team_id),
                                max(p.white_team_id, p.black_team_id),
                            )
                        )
                ...
                if lone_pairings.exists():
                    round_data["matches"][1] = (
                        game_ids  # Single "match" for lone tournaments
                    )
                else:
...
class TeamProfileView(LeagueView):
    def view(self, team_number):
        ...
        show_gender = can_manage_team or self.request.user.has_perm(
            "tournament.view_dashboard", self.league
        )
...
class TeamCreateView(LoginRequiredMixin, SeasonView):
    def view(self):
        ...
        if not captain_registration:
            ...
            any_registration = Registration.objects.filter(
                player=self.player, season=self.season, status="approved"
            ).first()
            ...
            if any_registration:
                ...
                raise Http404(
                    "You must have used a captain invite code during registration to create a team"
                )
            else:
    ...
    def view_post(self):
        ...
        form = TeamCreateForm(self.request.POST, season=self.season, player=self.player)
...
class KnockoutBracketView(SeasonView):
    ...
    def view(self):
        ...
        for round_num in range(1, display_rounds + 1):
            ...
            teams_remaining = (
                bracket.bracket_size // (2 ** (round_num - 1)) if bracket else 2
            )
            ...
            if round_obj:
                ...
                if not pairings.exists():
                    ...
                    if (
                        self.league.competitor_type == "team"
                        and bracket
                        and bracket.matches_per_stage > 1
                    ):
                        ...
                        for pairing in pairings:
                            if pairing.black_team:  # Skip byes (handle separately)
                                team_key = tuple(
                                    sorted(
                                        [pairing.white_team.id, pairing.black_team.id]
                                    )
                                )
                                team_pair_groups[team_key].append(pairing)
                                # Use the first pairing as primary (lowest pairing_order)
                                if (
                                    team_key not in team_pair_primary
                                    or pairing.pairing_order
                                    < team_pair_primary[team_key].pairing_order
                                ):
                        ...
                        for pairing in pairings:
                            if pairing.black_team_id is None:
                                seeding = pairing.white_team.knockoutseeding_set.filter(
                                    bracket=bracket
                                ).first()
                                matches.append(
                                    {
                        ...
                        logger.info(
                            f"Processing {len(team_pair_primary)} unique team pairs for multi-match bracket"
                        )
                        for team_key, primary_pairing in team_pair_primary.items():
                            ...
                            white_seeding = (
                                pairing.white_team.knockoutseeding_set.filter(
                                    bracket=bracket
                                ).first()
                            )
                            black_seeding = (
                                pairing.black_team.knockoutseeding_set.filter(
                                    bracket=bracket
                                ).first()
                            )

                            # For multi-match tournaments, aggregate scores across all matches for this team pair
                            aggregated_scores = self._get_aggregated_team_pair_scores(
                                pairing, round_obj
                            )
                            competitor1_score = aggregated_scores["white_total"]
                            competitor2_score = aggregated_scores["black_total"]
                            existing_matches_completed = aggregated_scores[
                                "all_completed"
                            ]
                            has_manual_tiebreak = aggregated_scores[
                                "has_manual_tiebreak"
                            ]
                            ...
                            if has_manual_tiebreak:
                                ...
                                team_pair_pairings = (
                                    ...
                                    .filter(
                                        models.Q(
                                            white_team=pairing.white_team,
                                            black_team=pairing.black_team,
                                        )
                                        | models.Q(
                                ...
                                if manual_tiebreak_val is not None:
                                    ...
                                    competitor1_won = (
                                        competitor1_score > competitor2_score
                                    )
                                    competitor2_won = (
                                        competitor2_score > competitor1_score
                                    )
                            ...
                            for match_pairing in team_pair_pairings:
                                ...
                                if match_pairing.white_team == pairing.white_team:
                                    match_scores.append(
                                        {
                                            "white_score": match_pairing.white_points
                                            or 0,
                                            "black_score": match_pairing.black_points
                                            or 0,
                                        }
                                    )
                                else:
                                    ...
                                    match_scores.append(
                                        {
                                            "white_score": match_pairing.black_points
                                            or 0,
                                            "black_score": match_pairing.white_points
                                            or 0,
                                        }
                                    )
                            ...
                            has_any_results = (
                                competitor1_score > 0 or competitor2_score > 0
                            )
                            ...
                            matches.append(
                                {
                                    ...
                                    "seed1": white_seeding.seed_number
                                    if white_seeding
                                    else None,
                                    "seed2": black_seeding.seed_number
                                    if black_seeding
                                    else None,
                                    ...
                                    "has_partial_results": has_any_results
                                    and not all_completed,  # Show partial if we have results but not all matches are completed
                                    "manual_tiebreak": has_manual_tiebreak,
                                    "round_number": round_obj.number,
                                    "pairing_id": pairing.id,  # Add pairing ID for admin links
                                    "needs_tiebreak": is_tie
                                    and not has_manual_tiebreak
                                    and all_completed,
                                    "pairing_order": pairing.pairing_order,
                        ...
                        logger.info(
                            f"Processing ALL {pairings.count()} pairings for single-match tournament"
                        )
                        for pairing in pairings:
                            if self.league.competitor_type == "team":
                                if pairing.black_team_id is None:
                                    ...
                                    all_completed = self._is_team_match_completed(
                                        pairing
                                    )
                                    has_manual_tiebreak = (
                                        pairing.manual_tiebreak_value is not None
                                    )
                                    ...
                                    if pairing.manual_tiebreak_value is not None:
                                        ...
                                        competitor1_won = (
                                            pairing.manual_tiebreak_value > 0
                                        )
                                        competitor2_won = (
                                            pairing.manual_tiebreak_value < 0
                                        )
                                        ...
                                        is_tie = (
                                            pairing.white_points == pairing.black_points
                                            and pairing.white_points is not None
                                        )
                ...
                if bracket and matches:
                    matches = self._sort_matches_for_bracket_display(
                        matches, round_num, bracket.bracket_size
                    )
            else:
                ...
                if round_num == scheduled_rounds + 1:
                    ...
                    if bracket and self.season.is_completed:
                        ...
                        final_advancement_records = KnockoutAdvancement.objects.filter(
                            bracket=bracket, to_stage="final"
                        ).order_by("date_created")
                        ...
                        if final_advancement_records.exists():
                            ...
                            last_round = (
                                Round.objects.filter(season=self.season)
                                .order_by("-number")
                                .first()
                            )
                            if last_round and self.league.competitor_type == "team":
                                if bracket.matches_per_stage > 1:
                                    ...
                                    actual_winners = _get_multi_match_winners(
                                        last_round, bracket
                                    )
                                else:
                                    # Single-match tournament: use individual pairing results
                        ...
                        matches.append(
                            {
                                ...
                                "is_tie": False,
                                "completed": winner
                                is not None,  # Completed if we have a winner
                                "manual_tiebreak": False,
                                "round_number": round_num,
                                "pairing_id": None,
                                "needs_tiebreak": False,
                                "pairing_order": i + 1,
                                "is_placeholder": winner
                                is None,  # Only placeholder if no winner
                                "winner_position": i
                                + 1,  # Position number for this winner
                            }
                        )
                else:
        ...
        advancements = (
            KnockoutAdvancement.objects.filter(bracket=bracket)
            .select_related(
    ...
    def _sort_matches_for_bracket_display(self, matches, round_number, bracket_size):
        ...
        if round_number == 1:
            return self._calculate_bracket_positions_for_first_round(
                matches, bracket_size
            )
        else:
    ...
    def _calculate_bracket_positions_for_first_round(self, matches, bracket_size):
        ...
        matches_without_seeds = [
            m
            for m in regular_matches
            if m.get("seed1") is None or m.get("seed2") is None
        ]
    ...
    def _get_aggregated_team_pair_scores(self, primary_pairing, round_obj):
        ...
        team_pair_pairings = (
            ...
            .filter(
                (
                    Q(white_team=primary_pairing.white_team)
                    & Q(black_team=primary_pairing.black_team)
                )
                | (
                    Q(white_team=primary_pairing.black_team)
                    & Q(black_team=primary_pairing.white_team)
                )
            )
            .exclude(black_team__isnull=True)
        )  # Exclude byes
...
class KnockoutSeasonLandingView(SeasonView):
    ...
    def view(self):
        ...
        elimination_summary = (
            self._get_elimination_summary() if not finalist_preview else None
        )

        # Get advancement status for admin users
        advancement_info = (
            self._get_advancement_info() if self.request.user.is_staff else None
        )
    ...
    def _get_bracket_status(self, bracket):
        ...
        if completed_rounds >= total_rounds:
            ...
            if self.league.competitor_type == "team":
                last_pairing = (
                    TeamPairing.objects.filter(round__season=self.season)
                    .order_by("-round__number")
                    .first()
                )
                if last_pairing and last_pairing.result:
                    champion = (
                        last_pairing.white_team
                        if last_pairing.white_points > last_pairing.black_points
                        else last_pairing.black_team
                    )
                else:
                    champion = None
            else:
                last_pairing = (
                    LonePlayerPairing.objects.filter(round__season=self.season)
                    .order_by("-round__number")
                    .first()
                )
                if last_pairing and last_pairing.result:
                    champion = (
                        last_pairing.white
                        if last_pairing.result in ["1-0", "1X-0F"]
                        else last_pairing.black
                    )
                else:
                    champion = None
        else:
    ...
    def _get_recent_results(self):
        ...
        if self.league.competitor_type == "team":
            recent_pairings = (
                TeamPairing.objects.filter(
                    round__season=self.season, round__is_completed=True
                )
                .select_related("white_team", "black_team")
                .prefetch_related("teamplayerpairing_set")
                .order_by("-round__number", "-pairing_order")[:20]
            )  # Get more records to filter from

            results = []
            for pairing in recent_pairings:
                if pairing.black_team_id and self._is_team_match_completed(
                    pairing
                ):  # Not a bye and completed
                    results.append(
                        {
                            "competitor1": pairing.white_team,
                            "competitor2": pairing.black_team,
                            "competitor1_score": pairing.white_points,
                            "competitor2_score": pairing.black_points,
                            "competitor1_won": pairing.white_points
                            > pairing.black_points,
                            "competitor2_won": pairing.black_points
                            > pairing.white_points,
                            "manual_tiebreak": pairing.manual_tiebreak_value
                            is not None,
                        }
                    )
    ...
    def _get_finalist_preview(self, bracket_status):
        ...
        current_round = (
            Round.objects.filter(season=self.season, is_completed=False)
            .order_by("number")
            .first()
        )
        ...
        if self.league.competitor_type == "team":
            pairings = (
                TeamPairing.objects.filter(round=current_round)
                .select_related("white_team", "black_team")
                .prefetch_related(
                    "white_team__knockoutseeding_set", "black_team__knockoutseeding_set"
                )
            )
    ...
    def _handle_advancement(self):
        ...
        try:
            ...
            if not last_completed_round:
                messages.error(
                    self.request, "No completed rounds found to advance from"
                )
                return redirect(self.request.path)
            ...
            if next_round:
                ...
                messages.success(
                    self.request, "Tournament has been completed - all rounds finished"
                )
...
class KnockoutPairingsView(PairingsView):
    ...
    def knockout_team_view(self, round_number=None, team_number=None):
        ...
        if bracket and context["round_number"]:
            ...
            # Add stage info
            total_pairings = (
                len(context["pairing_lists"]) if context["pairing_lists"] else 0
            )
    ...
    def get_knockout_multi_match_context(
        self,
        league_tag,
        season_tag,
        round_number,
        ...
        team_pairings = (
            ...
            .filter(Q(white_team=current_team) | Q(black_team=current_team))
            .order_by("pairing_order")
            .select_related("white_team", "black_team")
            .nocache()
        )
        ...
        for opponent_id, pairings in opponent_pairings.items():
            if opponent_id == "BYE":
                ...
                opponent_team = (
                    pairings[0].white_team
                    if pairings[0].white_team != current_team
                    else pairings[0].black_team
                )
        ...
        team_byes = list(
            TeamBye.objects.filter(
                round__number=round_number, round__season=self.season, team=current_team
            )
            .select_related("team")
            .nocache()
        )

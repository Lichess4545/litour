...
from heltour.tournament_core.structure import Match, Round, Tournament
...
def get_pairing_order_for_match(
    original_pairing_order: int, match_number: int, total_pairs: int
) -> int:
...
def generate_next_match_set(tournament: Tournament, round_number: int) -> Tournament:
    ...
    for original_pairing_order in range(1, total_pairs + 1):
        ...
        original_match = _find_match_by_pairing_order(
            round_obj, original_pairing_order, total_pairs
        )
        if original_match is None:
            raise ValueError(
                f"Could not find original match for pairing order {original_pairing_order}"
            )
...
def calculate_multi_match_winners(
    matches: List[Match],
    total_pairs: int,
    ...
    if not is_multi_match_stage_complete(matches, total_pairs, matches_per_stage):
        raise ValueError(
            "Cannot calculate winners - not all matches in stage are complete"
        )
    ...
    for original_pairing_order in range(1, total_pairs + 1):
        ...
        for match_number in range(1, matches_per_stage + 1):
            ...
            if match is None:
                raise ValueError(
                    f"Missing match {match_number} for pair {original_pairing_order}"
                )
        ...
        if winner_id is None:
            raise ValueError(
                f"No clear winner for pair {original_pairing_order} - may need manual tiebreak"
            )
...
def is_multi_match_stage_complete(
    matches: List[Match], total_pairs: int, matches_per_stage: int
) -> bool:
...
def get_multi_match_stage_status(
    matches: List[Match], total_pairs: int, matches_per_stage: int
) -> Dict:
    ...
    completed_current = _count_completed_matches_for_match_number(
        matches, current_match_number, total_pairs
    )
    ...
    return {
        ...
        "stage_complete": is_multi_match_stage_complete(
            matches, total_pairs, matches_per_stage
        ),
    }
...
def _get_current_match_number_from_matches(
    matches: List[Match], total_pairs: int
) -> int:
...
def _all_teams_completed_match(
    round_obj: Round, match_number: int, total_pairs: int
) -> bool:
    """Check if all teams completed a specific match number."""
    completed_count = _count_completed_matches_for_match_number(
        round_obj.matches, match_number, total_pairs
    )
    return completed_count == total_pairs


def _count_completed_matches_for_match_number(
    matches: List[Match], match_number: int, total_pairs: int
) -> int:
...
def _find_match_by_pairing_order(
    round_obj: Round, pairing_order: int, total_pairs: int
) -> Optional[Match]:
...
def _find_match_by_pairing_order_and_match_number(
    matches: List[Match],
    original_pairing_order: int,
    match_number: int,
    total_pairs: int,
) -> Optional[Match]:
    """Find a match by original pairing order and match number."""
    target_index = (
        get_pairing_order_for_match(original_pairing_order, match_number, total_pairs)
        - 1
    )
...
def _calculate_aggregate_winner(
    matches: List[Match], scoring: ScoringSystem
) -> Optional[int]:

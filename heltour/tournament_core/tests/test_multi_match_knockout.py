...
class TestGlobalSynchronization(unittest.TestCase):
    ...
    def test_cannot_generate_when_partial_complete(self):
        ...
        tournament = (
            builder.multi_match_knockout(matches_per_stage=2)
...
class TestColorSwitching(unittest.TestCase):
    ...
    def test_color_switching_in_return_matches(self):
        ...
        self.assertEqual(
            second_match.competitor1_id, team_b_id
        )  # Team B white (flipped)
        self.assertEqual(
            second_match.competitor2_id, team_a_id
        )  # Team A black (flipped)
    ...
    def test_multiple_pair_color_switching(self):
        ...
        self.assertEqual(
            round_obj.matches[2].competitor1_id, team_b_id
        )  # Team B white (flipped)
        self.assertEqual(
            round_obj.matches[2].competitor2_id, team_a_id
        )  # Team A black (flipped)
        self.assertEqual(
            round_obj.matches[3].competitor1_id, team_d_id
        )  # Team D white (flipped)
        self.assertEqual(
            round_obj.matches[3].competitor2_id, team_c_id
        )  # Team C black (flipped)
...
class TestBasicMultiMatchWinnerCalculation(unittest.TestCase):
    ...
    def test_simple_two_match_winner(self):
        ...
        round_obj = tournament.rounds[0]
        winners = calculate_multi_match_winners(
            round_obj.matches, 1, 2, tournament.scoring
        )
